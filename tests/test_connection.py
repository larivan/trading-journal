...
class TestTransaction:
    def test_commit_on_success(self, trade_deps):
        ...
        with transaction() as conn:
            trade_id = create_trade(_trade_payload(trade_deps), conn=conn)

        assert get_trade_by_id(trade_id) is not None
